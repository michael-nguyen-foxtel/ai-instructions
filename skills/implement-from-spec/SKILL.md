...
1. **Detect worktree** — `git rev-parse --show-toplevel` and `git worktree list`
   - If you're already in a worktree (path differs from the main worktree), you're already on the right branch. **Skip straight to step 5** (dependency check). Do NOT create or switch branches.
...
5. **Check if dependencies are stale**:
   - If `node_modules` doesn't exist: run the repo's install command
   - Otherwise compare the lockfile: `git diff HEAD@{1} --name-only | grep -E 'package-lock\.json|yarn\.lock|pnpm-lock\.yaml'`
   - If the lockfile changed on pull: run the repo's install command
